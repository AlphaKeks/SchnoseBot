...
pub(crate) async fn execute(mut data: InteractionData<'_>) -> anyhow::Result<()> {
	...
	let (tp, pro) = join_all([
		get_wr(&map_identifier, &mode, true, course, &data.req_client),
		get_wr(&map_identifier, &mode, false, course, &data.req_client),
	])
	.await
	.into_iter()
	.collect_tuple()
	...
	if let (&Err(_), &Err(_)) = (&tp, &pro) {
		return data.reply(Message("No BWRs found 😔.")).await;
	}
