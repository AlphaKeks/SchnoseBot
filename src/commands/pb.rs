...
pub(crate) async fn execute(mut data: InteractionData<'_>) -> anyhow::Result<()> {
	...
	let (tp, pro) = join_all([
		get_pb(&player, &map_identifier, &mode, true, 0, &data.req_client),
		get_pb(&player, &map_identifier, &mode, false, 0, &data.req_client),
	])
	.await
	.into_iter()
	.collect_tuple()
	...
	if let (&Err(_), &Err(_)) = (&tp, &pro) {
		return data.reply(Message("No PBs found 😔.")).await;
	}
